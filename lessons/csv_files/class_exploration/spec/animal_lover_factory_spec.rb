...
RSpec.describe AnimalLoverFactory do
  ...
  it "should parse csv to create animal lover objects" do
    ...
    factory.create_animal_lovers("./data/animal_lovers.csv")

    expect(factory.animal_lovers.count).to eq(6) 
    expect(factory.animal_lovers).to be_all(AnimalLover)
    first = factory.animal_lovers.first
